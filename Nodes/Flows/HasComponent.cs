    ...
    public class HasComponent : ValueNode {
		...
		protected override string GenerateValueCode() {
			if(entity.isAssigned && componentType.isAssigned) {
				...
				if(ecbName != null) {
					return ecbName.CGInvoke(nameof(EntityManager.HasComponent), new[] { componentType.type }, entity.CGValue());
				}
				else {
					return ecbName.CGInvoke(nameof(SystemAPI.HasComponent), new[] { componentType.type }, entity.CGValue());
				}
			}
			return null;
		}
